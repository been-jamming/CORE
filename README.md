...
To install, copy the executable created after building to `/usr/bin`.
...
This is the proof that for any predicate `P`, if for no `X` is `P(X)` true, then for all `X`, `P(X)` is not true. The proof starts out with the `prove` keyword,
...
predicate `P`, which depends on 1 object. What this means is that the proof will work for any prediate `P` we are given. Throughout the proof, we will be
able to refer to `P`, but we won't know anything about what `P` says since the proof should work for all such predicates. Finally, after the colon we state what
we are trying to prove. When we write a predicate like this, `~` means "not", `^` means "there exists", `->` means "implies", and `*` means "for all".
...
When proving a statement that looks like `~A`, a common strategy is to suppose `A` was true and to prove a contradiction. This is how the next keyword, `assume`,
works in the proof. Since our goal looks like `~P(X)`, we are assuming `P(X)` by storing it into the variable `P_X`, and our new goal becomes to prove
`false`. Obviously false isn't actually true, but this is how we structure a proof by contradiction in CORE.

Let's recap. We have our predicate `P`, depending on one variable which we know nothing about. We have our variable `not_exists`, which says that there doesn't
...
globally stores the statement that for any predicate `P` depending on 1 variable, `~^X(P(X)) -> *X(~P(X))`!
